...
app.post("/lists/:todoListId/todos/:todoId/destroy", (req, res, next) => {
  let { todoListId, todoId } = { ...req.params }; //get the IDs from the req and store as todoListId and todoId JE
  
  let todoList = loadTodoList(+todoListId, req.session.todoLists);//the + is coercing the id string into a number JE
  if (!todoList) {
    next(new Error("Not found.")); //run error handler if the list doesn't exist JE
  } else {
    let todo = loadTodo(+todoListId, +todoId, req.session.todoLists);
    if (!todo) {
      next(new Error("Not found."));
...
app.post("/lists/:todoListId/complete_all", (req, res, next) => {
  ...
  if(!todoList) {
    ...
    req.flash("success", "The todo list has been completed.");
    res.redirect(`/lists/${todoListId}`);
  }
});
...
app.post("/lists/:todoListId/todos", 
  [
    body("todoTitle")
      ...
      .isLength({ max: 100 })
      .withMessage("todo item title must be between 1 and 100 characters")
  ],
  (req, res, next) => {
    let todoListId = req.params.todoListId;
    let todoList = loadTodoList(+todoListId, req.session.todoLists);
    if (!todoList) {
      ...
      if (!errors.isEmpty()) {
        ...
        todoList.add(todo);
        req.flash("success", "A new Todo has been created");
        res.redirect(`/lists/${todoListId}`);
      }
    }
  }
...
app.post("/lists/:todoListId/destroy", (req, res, next) => {
  ...
  if (index === -1) {
    ...
    req.flash("success", "the list has been deleted.");
    res.redirect("/lists");
  }
});
